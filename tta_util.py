...
def shave_a2b(a, b):
    ...
    assert ((a.shape[r] - b.shape[r]) %
            2 == 0) and ((a.shape[c] - b.shape[c]) % 2 == 0)
    # Calculate the shaving of each dimension
    shave_r, shave_c = max(
        0, a.shape[r] - b.shape[r]), max(0, a.shape[c] - b.shape[c])
...
def create_gradient_map(im, window=5, percent=.97):
    ...
    gx_pad, gy_pad, gmag = pad_edges(gx, int(window)), pad_edges(
        gy, int(window)), pad_edges(gmag, int(window))
    lm_x, lm_y, lm_gmag = clip_extreme(gx_pad, percent), clip_extreme(
        gy_pad, percent), clip_extreme(gmag, percent)
    ...
    loss_map = convolve2d(grads_comb, np.ones(
        shape=(window, window)), 'same') / (window ** 2)
...
def create_probability_map(loss_map, crop):
    ...
    blurred = convolve2d(loss_map, np.ones(
        [crop // 2, crop // 2]), 'same') / ((crop // 2) ** 2)
    # Zero pad s.t. probabilities are NNZ only in valid crop centers
    prob_map = pad_edges(blurred, crop // 2)
    # Normalize to sum to 1
    prob_vec = prob_map.flatten() / prob_map.sum() if prob_map.sum(
    ) != 0 else np.ones_like(prob_map.flatten()) / prob_map.flatten().shape[0]
    return prob_vec
...
def clip_extreme(im, percent):
    ...
    v_max = im_sorted[pivot + 1] if im_sorted[pivot +
                                              1] > v_min else v_min + 10e-6
...
def create_gaussian(size, sigma1, sigma2=-1, is_tensor=False):
    """Return a Gaussian"""
    func1 = [np.exp(-z ** 2 / (2 * sigma1 ** 2)) / np.sqrt(2 * np.pi * sigma1 ** 2)
             for z in range(-size // 2 + 1, size // 2 + 1)]
    func2 = func1 if sigma2 == -1 else [np.exp(-z ** 2 / (2 * sigma2 ** 2)) / np.sqrt(
        2 * np.pi * sigma2 ** 2) for z in range(-size // 2 + 1, size // 2 + 1)]
...
def kernel_shift(kernel, sf):
    ...
    wanted_center_of_mass = np.array(
        kernel.shape) // 2 + 0.5 * (np.array(sf) - (np.array(kernel.shape) % 2))
...
def save_final_kernel(k_2, conf):
    ...
    sio.savemat(os.path.join(conf.output_dir, '%s_kernel_x2.mat' %
                conf.abs_img_name), {'Kernel': k_2})
...
def calc_curr_k(G_DW_params):
    ...
    for ind, w in enumerate(G_DW_params):
        # if ind == 0 else F.conv2d(curr_k, w)
